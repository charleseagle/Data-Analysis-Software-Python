...
class Fr_SpectRefine(DataOperationFrame,Tmp_PickRange):
	...
	def __ginit(self):		
		self.gettmpdata('savename').set('spectrarefined');
	...
	def analyze(self,igroup):
		...
		for k in spectra.keys():
			...
			spectnew.refine(resolution);
			spectnew.log({"Operation":"refine","resolution":resolution,"xmin":xmin,"xmax":xmax});
