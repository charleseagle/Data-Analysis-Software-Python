...
class Fr_SpectFixJump(DataOperationFrame,Tmp_PickRange):
	...
	def __ginit(self):
		self.gettmpdata('savename').set('jump');
	...
	def analyze(self,igroup):
		...
		for k in spectra.keys():
			...
			spectnew.fixjump(xmin=xmin,xmax=xmax,norder=norder,npoints=npoints);
			spectnew.log({"Operation":"fixjump","xmin":xmin,"xmax":xmax,"norder":norder,"npoints":npoints});
